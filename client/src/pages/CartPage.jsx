...
const CartPage = () => {
  // 🟢 CHANGE 1: Import decreaseCartItem
  const { cart, addToCart, removeFromCart, decreaseCartItem, clearCart } = useCart();
  ...
  const groupedItems = useMemo(() => {
    ...
    cart.forEach(item => {
      if (!groups[item._id]) {
        groups[item._id] = { ...item, qty: 0 };
      }
      groups[item._id].qty += 1;
    });
                            ...
                            <div key={item._id} className="group relative flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 rounded-xl bg-white shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-transparent hover:shadow-md hover:border-[#6354e8]/20 transition-all duration-300">
                                ...
                                <div className="flex items-center gap-6 mt-4 sm:mt-0 w-full sm:w-auto justify-between sm:justify-end">
                                    <div className="flex items-center rounded-lg bg-gray-50 border border-gray-200 p-1">
                                        ...
                                        <input className="w-10 bg-transparent text-center text-sm font-semibold border-none focus:ring-0 p-0 text-gray-900" readOnly type="text" value={item.qty} />
